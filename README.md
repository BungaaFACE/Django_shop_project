...
- Система личных клиентов для рассылки.  
- Система общих темплейтов для отправки рассылок.  
- Система периодических рассылок.  
- Роль менеджера, которая может выключать рассылки и блокировать пользователей.
...
### Менеджер  
Имеет доступ включению и выключению рассылок, блокированию пользователей.  
Создание менеджера --> `python manage.py cmanager`  
Логин пароль менеджера --> manager@gmail.com - manager  
...
### Шаг восьмой  
- Знакомство с django_tables2 и применение их в отображении инструментов рассылки.  
- Реализован интерфейс заполнения рассылок, то есть CRUD-механизм для управления рассылками. В него входит список клиентов, список темплейтов письма, список рассылок.  
- Реализовано логирование отправлений писем с рассылками.  
- Знакомство с django-crontab и реализация автоматических рассылок на его основе.  
- Добавлена возможность вручную запускать рассылку - `python manage.py send_distributions`  
- Добавление прав доступа. Пользователь может RUD (READ, UPDATE, DELETE) только своих клиентов, видеть темплейты всех пользователей, но изменять и удалять только свои. RUD рассылок также доступен только создателю.  
- Создана группа менеджеров. Права менеджера:  
...
- Настроена административная панель для контент менеджера.  
- Изменена главная страница. Теперь отображена следующая информация:  
  - Количество рассылок всего.  
  - Количество активных рассылок.  
  - Количество уникальных клиентов для рассылок.  
  - 3 случайные статьи из блога.  
UPD:  
- Изменена система рассылки, теперь можно указать сразу несколько получателей в одну рассылку.  
- Исправлен расчет следующего дня отправки - теперь дата всегда будет +1 день от последней отправки.  
